...
<body>
  ...
  <div class="container">
    ...
    <div class="card">
      <div class="tabs">
        <button class="tab-btn active" onclick="switchTab('single')" id="tab-single">单条/域名检测</button>
        <button class="tab-btn" onclick="switchTab('batch')" id="tab-batch">批量检测</button>
      </div>
      ...
      <div class="form-section">
        ...
        <div id="batch-input-container" style="display: none;">
          <label for="batch-ips" class="form-label">📦 批量输入 IP (每行一个)</label>
          <div class="input-group" style="display: block;">
            <textarea id="batch-ips" class="form-textarea" placeholder="1.1.1.1:443&#10;2.2.2.2:8443&#10;3.3.3.3:2053"></textarea>
            <div style="margin-top: 15px; display: flex; gap: 10px;">
              <button id="batchCheckBtn" class="btn btn-primary" onclick="startBatchCheck()">
                <span class="btn-text">开始批量检测</span>
                <div class="loading-spinner" style="display: none;"></div>
              </button>
              <button id="exportBtn" class="btn btn-secondary" onclick="exportValidIPs()" style="display:none;">
                📋 导出有效IP
              </button>
            </div>
          </div>
          ...
          <div id="batch-stats" style="display: none;">
            <div class="batch-stat">
              <div class="stat-item" style="color: var(--text-primary);">
                <span class="stat-val" id="stat-total">0</span>
                总数
              </div>
              <div class="stat-item" style="color: var(--success-color); background: #f0fff4;">
                <span class="stat-val" id="stat-valid">0</span>
                有效
              </div>
              <div class="stat-item" style="color: var(--error-color); background: #fff5f5;">
                <span class="stat-val" id="stat-invalid">0</span>
                无效
              </div>
            </div>
...
&nbsp;&nbsp;"timestamp": "2025-06-03T17:27:52.946Z" // 检查时间<br>
  ...
  <script>
    ...
    async function startBatchCheck() {
      ...
      if (lines.length === 0) {
        showToast('请输入至少一个IP地址');
        return;
      }

      // 检查时间戳
      if (calculateTimestamp() !== pageLoadTimestamp) {
      ...
      async function worker() {
        while (queue.length > 0) {
          ...
          try {
            ...
            if (data.success) {
              valid++;
              batchValidIPs.push(ip); // 保存原始输入的IP格式
              document.getElementById('stat-valid').textContent = valid;
              logItem.innerHTML = \`<span class="log-success">✔ \${ip} 有效 (\${data.responseTime}ms) - \${data.colo}</span>\`;
            } else {
              invalid++;
              document.getElementById('stat-invalid').textContent = invalid;
              logItem.innerHTML = \`<span class="log-error">✘ \${ip} 无效 - \${data.message || '连接失败'}</span>\`;
            }
          } catch (err) {
            processed++;
            invalid++;
            document.getElementById('stat-invalid').textContent = invalid;
            logItem.innerHTML = \`<span class="log-error">✘ \${ip} 错误 - \${err.message}</span>\`;
          }
        }
      }

      // 启动并发workers
      await Promise.all(Array(Math.min(lines.length, CONCURRENCY)).fill().map(worker));
      ...
      if (batchValidIPs.length > 0) {
        exportBtn.style.display = 'inline-flex';
      }
      
      showToast(\`批量检测完成: \${valid}个有效\`);
    }

    function exportValidIPs() {
      if (batchValidIPs.length === 0) {
        showToast('没有可导出的有效IP');
        return;
      }
      copyToClipboard(batchValidIPs.join('\\n'), null);
    }
